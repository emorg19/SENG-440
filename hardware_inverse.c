...
{
	register int_type max = augmented[0][col];
...
// function for using the hardware instruction
void hardware_vanish_column(int_type[size][size*2] a, int column) 
{   
	asm (
			"cvan %0, %1\n\t"
			: "=r" (a)
...
void eliminate() 
{
	...
	for(int k = 0; k < size; k++) 
	{
		...
		// using hardware assist to vanish each column
		hardware_vanish_column(augmented, k);
	}
}
